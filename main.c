...
/*   Updated: 2022/03/02 11:00:22 by conguyen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_image(t_fdf *fdf)
{
	...
	for (int x = 0; x < fdf->map.height; x++)
	{
		for (y = 0; y < fdf->map.width -1; y++)
		{
			if (y == 0)
			{
				fdf->pixel.x1 = ((y * 35 + 450) - (x * 40)) * fdf->flags.zoom_flag;
				fdf->pixel.y1 = ((x * 35 + 50) + (y * 20)) * fdf->flags.zoom_flag;
			}
			if (fdf->map.map[x][y] < fdf->map.map[x][y + 1])
			{
				if ((((y + 1) * 35 + 450) - (x * 40)) * fdf->flags.zoom_flag < 1200)
					fdf->pixel.x2 = (((y + 1) * 35 + 450) - (x * 40)) * fdf->flags.zoom_flag;
				else
					fdf->pixel.x2 = 1199;
				if ((((x * 35 + 50) + ((y + 1) * 20)) - (fdf->map.map[x][y + 1] * 4)) * fdf->flags.zoom_flag < 1000)
			...
			else if (fdf->map.map[x][y] > fdf->map.map[x][y + 1])
			{
				if ((((y + 1) * 35 + 450) - (x * 40)) * fdf->flags.zoom_flag < 1200)
					...
					fdf->pixel.x2 = 1199;
				if (((x * 35 + 50) + ((y + 1) * 20)) * fdf->flags.zoom_flag < 1000)
					fdf->pixel.y2 = ((x * 35 + 50) + ((y + 1) * 20)) * fdf->flags.zoom_flag;
				else 
					fdf->pixel.y2 = 999;
			}
			else
			{
				if ((((y + 1) * 35 + 450) - (x * 40)) * fdf->flags.zoom_flag < 1200)
					...
					fdf->pixel.x2 = 1199;
				if (fdf->pixel.y1 + (20 * fdf->flags.zoom_flag) < 1000)
		...
		for (y = 0; y < fdf->map.height - 1; y++)
		{
			...
			else if (fdf->map.map[y][x] > fdf->map.map[y + 1][x])
			{
				...
				if ((((y + 1) * 35 + 50) + (x * 20)) * fdf->flags.zoom_flag < 1000)
					fdf->pixel.y2 = (((y + 1) * 35 + 50) + (x * 20)) * fdf->flags.zoom_flag;
				else
					fdf->pixel.y2 = 999;
			}
			else
...
void	initialize_mlx(t_fdf *fdf, int height, int width)
{
	...
	// mlx_mouse_hook(fdf->mlx.win, &mouse_event, fdf);
